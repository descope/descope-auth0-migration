...
def fetch_auth0_users():
    ...
    response = requests.get(f"https://{AUTH0_TENANT_ID}.us.auth0.com/api/v2/users", headers=headers)
...
def process_users(exported_users, api_response_users):
    ...
    """
    api_user_map = {api_user["user_id"].split('|')[-1]: api_user for api_user in api_response_users}
